...
const Productnew = () =>{
  ...
  const handleGuestLogin = () => {
    let item = product;
    let attributesdata = selectedAttributes;
    let cartItem = {};
    ...
    if(item.product_type === 'variable'){
        cartItem = {
            ...
            shipping_charges: attributesdata[0].shipping_charges || 0, // Assuming attributesdata.shipping_charges is the shipping charges
  ...
  const handleRegularLogin = () => {
    ...
    // Find the index of the image URL in the productImages array
    const currentIndex = product.productImages.findIndex((img) => {
        //console.log(img.split('_')[1])
        //console.log(imageUrl.split('_')[1])
        return (img.split('_')[1] === imageUrl.split('_')[1])
    });
    // console.log(currentIndex)
    // Move the slider to the corresponding index
    sliderRef33.current.slickGoTo(currentIndex);
        ...
        if(item.product_type === 'simple'){
            ...
            cartItem = {
                ...
                others: {
                    ...
                    selectedValues: attributesdata[0].shipping_charges || 0 // Assuming attributesdata.selectedValues is the selected values object
                },
                vendor_id:item.vendor_id,
                vendorName:item.vendorName
            };
                    ...
                    <div className='col-lg-6 col-md-12 col-sm-12'>
                            ...
                            <div className='d-flex justify-content-between'>
                                <div className=''>
                                    {product?.price > 0 && (product?.price - product?.max_price!=0) ? 
                                        (<p className='product_page_price'>₹ {(product?.price)} - ₹ {(product?.max_price) }</p>) : (<p className='product_page_price'>₹ {(product?.price)}</p>)
                                    }
                                </div>
                            </div>
                        </div>
                        ...
                        {product.quantityTypes && (Object.keys(JSON.parse(product.quantityTypes)).length > 0) && <div className='row'>
                            ...
                            <div className='col-12 d-flex flex-row justify-content-start'>
                                ...
                                <p className='product_page_price'>₹ {productPrice}</p>
                            </div>
                        </div>}
                        ...
                        <div className='d-flex flex-row justify-content-start'>
                            <div className='col-12'>
                                ...
                                <button type="button" className="buynow_button_product_page" style={{border:"1px solid green",background:"white",color:"green",outline:'0'}} onClick={() => addToCart(product, selectedAttributes)} disabled={pincodeVerifyDisable.cartButton === 1}>Add to cart</button>
                                {isAddedToCart && (
                                ...
                                {
                                    user?.ID ? (
                                        <button className='buynow_button_product_page' disabled={pincodeVerifyDisable.buyButton === 1} onClick={() => addToBuyCartItems(product, selectedAttributes)}>Buy Now</button>
                                    
                                )
                                : 
                                (
                                    <button className='buynow_button_product_page' disabled={pincodeVerifyDisable.buyButton === 1} onClick={() =>  openModal()}>Buy Now</button>
                                ) 
                                }    
                            </div>
                        </div>
        ...
        <section>
            <div className='container mb-5'>
                <div className='row'>
                    ...
                    {/* React slick slider */}
                    <Slider ref={sliderRef} {...settings} className='reviews_slider_product variable_related_products' >
