...
const reduceText = (text) => text.length > maxDescriptionLength ? `${text.substring(0, maxDescriptionLength)}...` : text;



export default function BasicVichaarDetailsScreen() {
    ...
    useEffect(() => {
        Promise.all([
            Apicalls.get('basicVichaar/' + window.location.pathname.split('/')[2]),
            Apicalls.get('basicVichaar/hot/1'),
            Apicalls.get('basicVichaar/trending/1'),
        ]).then(([data, hotBlogsData, trendingBlogsData]) => {
            setBlog(data.data)
            setHotBlogs(hotBlogsData.data)
            setTrendingBlogs(trendingBlogsData.data)
                ...
                <div className='container'>
                    <div className='row'>
                        <div className='col-lg-8 col-md-8 col-sm-12'>
                            ...
                            {blog.img_urls && <img src={(post_url + JSON.parse(blog.img_urls)[0])} alt={blogtitlepage_img1} className='img-fluid' />}
                            {blog.description}
                        ...
                        <div className='col-lg-4 col-md-4 col-sm-12 d-none d-md-block'>
                            ...
                            <div className='row'>
                                ...
                                {
                                    hotBlogs.map((item) => {
                                        return <div className='row mb-2' style={{ alignItems: 'center' }}>
                                            <div className='col-4'>
                                                <img src={post_url + JSON.parse(item.img_urls)[0]} alt={item.heading} className='img-fluid' />
                                            </div>
                                            <div className='col-8'>
                                                <p className='news_para1'>{reduceText(item.heading)}</p>
                                            </div>
                                        </div>
                                    })
                                }
                            ...
                            <div className='row'>
                                ...
                                </div>
                                {
                                    trendingBlogs.map((item) => {
                                        console.log(reduceText(item.description), item)
                                        return <div className='row mb-2' style={{ alignItems: 'center' }}>
                                            <div className='col-4'>
                                                <img src={post_url + JSON.parse(item.img_urls)[0]} alt={item.title} className='img-fluid' />
                                            </div>
